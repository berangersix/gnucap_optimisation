...
private:
  ...
  bool		test_alignement_16byte(void* ptr);
  double	dot_prod_sse(double *p, double *q,int size);
  std::complex<double>	dot_prod_sse(std::complex<double>*, std::complex<double>* q,int size);
...
T& BSMATRIX<T>::subtract_dot_product(int rr, int cc, int dd)
{
  ...
  T& dot = m(rr, cc);
  if (len > 0) {
    T* __attribute__((aligned(16))) row = &(l(rr,kk));
    T* __attribute__((aligned(16))) col = &(u(kk,cc));
	if(len % 2 == 0 )//not override with -i-1
	{ 
	  dot -= dot_prod_sse(row,col,len); 
	}
	else{
	  dot -= dot_prod_sse(row,col,len-1) -  row[-(len-1)] * col[len-1];
	}
  }else{
  }
  return dot;
...
template <class T>
bool  BSMATRIX<T>::test_alignement_16byte(void* ptr){
    //test alignment
    if ( !( (long)ptr % 16 )){
        //printf("aligned\n");
        return true;
...
template <class T>
std::complex<double>  BSMATRIX<T>::dot_prod_sse(std::complex<double>* p, std::complex<double>* q,int size)
{
...
template <class T>
double BSMATRIX<T>::dot_prod_sse(double *p, double *q,int size)
{


    __m128d final = _mm_setzero_pd ();


    //end debug

    for(int i=0;i<size;i+=2){
	//need to be redone when aligned vector
        __m128d q_1 = _mm_loadu_pd(&q[i]);
	q_1 = _mm_shuffle_pd (q_1, q_1, 1); 
        __m128d p_1 =  _mm_loadu_pd(&p[-i-1]);
        // The flag 0xFF instructs the CPU to perform full dot product.
        __m128d dp_1 = _mm_dp_pd(p_1, q_1, 0xFF);
        // Add all subproducts
        final = _mm_add_pd(final,dp_1);
    }
...
void BSMATRIX<T>::allocate()
{
  ...
  size_t size_s = (size()+1) * sizeof(T*); // this is how many bytes you need in the aligned buffer
  size_t size_nz = _nzcount * sizeof(T); // this is how many bytes you need in the aligned buffer
  size_t align = 16;  // this is the alignment boundary


  _colptr =(T**) _mm_malloc(size_s,16);
  _rowptr =(T**) _mm_malloc(size_s,16);
  _diaptr =(T**) _mm_malloc(size_s,16);
  _space =(T*) _mm_malloc(size_nz,16);
/*
